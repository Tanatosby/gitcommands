...
<code> git add <filename.txt> // to add a specific file
 </code> 
 
<code>git add . </code> =>to add all the files that are new or changed in the current directory <br/>
<code> git add -A </code> //to add all changes in the entire working tree, from the root of the repository, regardless of where you are in the directory structure </code>
...
[- -hard uses two dashes (- -), not a single dash (-).]
Note- Using git reset –hard HEAD permanently deletes all uncommitted changes in both the staging area and working directory. Once executed, these changes cannot be recovered. Make sure to commit or back up your work before executing this command.
...
<code>git branch</code> (to list branches) <br/>
<code>git branch <new-branch> </code> (to create a new branch) <br/>
<code>git branch -d <branch-name></code> (to delete a branch) <br/>
...
Description: It allows you to move between branches or even create and switch to a new branch in a simple way.

Syntax: git switch <branch-name>

git restore

Description: It discards or resets changes in files by restoring them either from the staging area or from a specific commit.

Syntax: git restore <file>

To unstage a file (remove from staging area):
...
Description: This command is used to authenticate your GitHub account in the GitHub CLI. It lets you log in with either a web browser or a token, so you can securely use GitHub features (like creating repositories, managing pull requests, or checking issues) directly from the terminal.
